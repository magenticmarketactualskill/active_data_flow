...
The `submoduler-core-submoduler_child` gem is used by subgems and submodules in the active_data_flow project. It provides tools for managing child components in a monorepo environment, enabling them to reference and interact with the parent repository.

**Key Distinction**: 
- **Parent gem** (`submoduler_parent`) - Used by active_data_flow (the parent repository)
- **Child gem** (`submoduler_child`) - Used by subgems and submodules (child components)

## Installation

### In Subgems

All subgems include this in their Gemfile:
...
The submoduler_child gem enables subgems to:
...
Subgems use symbolic links to reference parent documentation:

```bash
# From subgem directory
submoduler_master -> ../../../  # Link to parent repository
```

This allows subgems to access parent `.kiro/` documentation and configuration.
...
Subgems using submoduler_child follow this structure:
...
Through the submoduler_child gem, subgems can:
...
1. **Create directory**:
   ```bash
   mkdir -p subgems/active_data_flow-new-component
   cd subgems/active_data_flow-new-component
   ```
...
cd ../..
bundle exec rspec subgems/active_data_flow-new-component/spec
```
...
├── Gemfile references subgems via path
└── subgems/
...
- **Always include .submoduler.ini** in subgems
